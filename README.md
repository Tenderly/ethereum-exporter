...
| eth_first_archive_block_number | Earliest block whose state the node still retains. Found by binary search over `eth_getBalance`, refreshed in the background once at startup and then on every `-ethfirstarchiveblockinterval` wall-clock boundary in UTC (default `1h`, so at 01:00, 02:00, ...). |
